...
1. **SQL Injection.** ZAP отримав `500 Internal Server Error` на ін'єкційний пошуковий запит. Це показує, що DAST добре знаходить проблеми, які проявляються через реальну HTTP-поведінку застосунку.
2. **Content Security Policy Header Not Set.** Відсутній CSP-заголовок. Для SPA це важливо, бо CSP зменшує наслідки XSS та помилок у роботі з HTML/JavaScript.
3. **Cross-Domain Misconfiguration.** ZAP виявив потенційно небезпечну міждоменну поведінку (cross-origin) на ресурсах застосунку. Це варто перевіряти в передрелізному середовищі (staging), бо CORS-помилки часто залежать від конфігурації під час виконання (runtime).
4. **Session ID in URL Rewrite.** Ідентифікатор сесії потрапляє в URL. Такі значення можуть залишатися в логах, історії браузера або referrer-заголовках.
5. **Missing Anti-clickjacking Header.** Відсутність `X-Frame-Options` або відповідної CSP-директиви може дозволити відкривати сторінку у frame та створювати clickjacking-сценарії.
6. **X-Content-Type-Options Header Missing.** Відсутній `X-Content-Type-Options: nosniff`. Це не найкритичніша проблема, але вона показує нестачу базового посилення захисту (hardening) для HTTP-відповідей.
